...
Cofnは、次の相反しやすい目標を一つの言語設計にまとめる実験的プログラミング言語です。
...
`Cofn`は現時点では**ワーキングタイトル**です。同名のRust crateなどが存在するため、公開前に名称・商標・package namespaceを再調査します。詳しくは[docs/NAMING.md](docs/NAMING.md)を参照してください。
...
- `cofn.law-evidence/v1`形式のJSON evidenceとassurance gate
- Cofn自身で書かれ、Stage 0からnative buildできるStage 1 compiler seed
- Python unit tests
- 13,500件の実装backlog
...
./bin/cofn run examples/pipeline.cofn
./bin/cofn run examples/science.cofn
```

型検査:

```bash
./bin/cofn check examples/ownership.cofn
```
...
Cofnで書かれたStage 1 compiler seedの検証:
...
```bash
./bin/cofn build examples/fibonacci_native.cofn --backend c -o build/fibonacci
./build/fibonacci
```
...
```bash
./bin/cofn build examples/fibonacci_native.cofn \
  -o build/fibonacci \
...
schema IDは`cofn.law-evidence/v1`です。source SHA-256、compiler version、assurance、model digest、case数、diagnostics、counterexamplesを含みます。`cofn check`と`cofn build`にも`--require-law-assurance`があり、要求より弱いevidenceを`L200`でrejectします。
...
- 13,500件あること
- IDが`COFN-00001`から`COFN-13500`まで連続すること
- IDが重複しないこと
- fingerprintが重複しないこと
- 各area fileに500件あること
- 必須cellが空でないこと
...
```text
cofn-lang/
├── bin/cofn                 # zero-install launcher
├── src/cofn/                # lexer, parser, checker, law checker, interpreter, C backend, CLI
├── bootstrap/                # Cofn-written Stage 1 seed and bootstrap gates
├── artifacts/                # checked law-evidence examples
├── examples/                 # runnable programs
├── tests/                    # Python unit tests and .cofn language tests
