...
Frostは、次の相反しやすい目標を一つの言語設計にまとめる実験的プログラミング言語です。
...
`Frost`は現時点では**ワーキングタイトル**です。同名のRust crateなどが存在するため、公開前に名称・商標・package namespaceを再調査します。詳しくは[docs/NAMING.md](docs/NAMING.md)を参照してください。
...
- `frost.law-evidence/v1`形式のJSON evidenceとassurance gate
- Frost自身で書かれ、Stage 0からnative buildできるStage 1 compiler seed
- Python unit tests
- 13,500件の実装backlog
...
./bin/frost run examples/pipeline.frost
./bin/frost run examples/science.frost
```

型検査:

```bash
./bin/frost check examples/ownership.frost
```

Monad則の検査:

```bash
./bin/frost laws examples/lawful_list_monad.frost
...
Frostで書かれたStage 1 compiler seedの検証:
...
```bash
./bin/frost build examples/fibonacci_native.frost --backend c -o build/fibonacci
./build/fibonacci
```
...
```bash
./bin/frost build examples/fibonacci_native.frost \
  -o build/fibonacci \
...
schema IDは`frost.law-evidence/v1`です。source SHA-256、compiler version、assurance、model digest、case数、diagnostics、counterexamplesを含みます。`frost check`と`frost build`にも`--require-law-assurance`があり、要求より弱いevidenceを`L200`でrejectします。
...
- 13,500件あること
- IDが`FROST-00001`から`FROST-13500`まで連続すること
- IDが重複しないこと
- fingerprintが重複しないこと
- 各area fileに500件あること
- 必須cellが空でないこと
...
```text
frost-lang/
├── bin/frost                 # zero-install launcher
├── src/frost/                # lexer, parser, checker, law checker, interpreter, C backend, CLI
├── bootstrap/                # Frost-written Stage 1 seed and bootstrap gates
├── artifacts/                # checked law-evidence examples
├── examples/                 # runnable programs
├── tests/                    # Python unit tests and .frost language tests
