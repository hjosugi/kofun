...
This directory holds the executable evidence for #620: the non-looping
Text/function portion of the frozen self-host profile lowers from
`kofun.selfhost-hir/v1` documents (`bootstrap/selfhost/hir-v1.md`) to
deterministic standalone C11. The lowering consumes only document
records — nodes, symbols, bindings, scopes, and the closed type table —
and never reparses source text or infers semantics from spelling.
...
- `trap_division.kofun` / `.stderr` — the explicit failure path: checked
  ...
  through every intervening call frame.
- `reject_missing_return.kofun` / `.hir` — a complete typed document
  whose non-`Void` function can finish without returning is invalid
  (exit 1, `E2S19`); the gate also drives #621/#622 constructs
  (mutation, loops, List and host builtins) through the lowering and
  requires the distinct `E2S10` unsupported classification (exit 3),
  never emitting partial C either way.
- `check-c11.sh` — the gate; run it with `make selfhost-profile` or
  directly. `sh bootstrap/selfhost/check-profile.sh --phase c11-text`
  (`make selfhost-c11`) is the completion check for the 31 profile rows
  whose `c11` cells the slice owns.
