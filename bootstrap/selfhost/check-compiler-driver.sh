...
# The loop slice takes the same two compiler paths. Its checked-in C proves one
# brace pair per loop block, a range whose ends are evaluated once into the
# enclosing scope, a bound name scoped to its own block and rebindable after it,
# and loops nested in each other and in branches. Executing it proves the body
# of a false `while` and of an empty range stayed unentered — both hold `1 // 0`.
...
for fixture in \
    ...
    bootstrap/selfhost/driver/corpus_reject_while_condition.kofun \
    bootstrap/selfhost/driver/corpus_reject_range_bound.kofun \
    bootstrap/selfhost/driver/corpus_reject_range_separator.kofun \
    bootstrap/selfhost/driver/corpus_reject_loop_shadow.kofun \
    bootstrap/selfhost/driver/corpus_reject_loop_scope.kofun \
    bootstrap/selfhost/driver/corpus_reject_loop_assignment.kofun \
    bootstrap/selfhost/driver/corpus_reject_else_after_loop.kofun
do
