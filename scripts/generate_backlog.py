...
AREAS: tuple[Area, ...] = (
    ...
    Area("packages", "Build Package and Registry", "tooling", (
        "kofun new", "kofun run", "kofun check", "kofun build", "kofun test", "kofun fmt", "kofun lint", "kofun doc", "kofun bench",
    ...
    Area("bootstrap", "Self Hosting and Bootstrap", "compiler", (
        "Stage 0 reference compiler", "Stage 1 Kofun frontend", "Stage 1 Kofun type checker", "Stage 1 ownership checker", "Stage 1 law checker",
...
PROFILES: dict[str, tuple[tuple[str, str, str, str], ...]] = {
    ...
    "tooling": (
        ("ux", "Define command and workflow UX", "Common workflows are direct, discoverable, and consistent with the single Kofun tool.", "UX review passes"),
...
def generate() -> dict[str, object]:
    ...
    for area_index, area in enumerate(AREAS, start=1):
        ...
        lines = [
            ...
            "",
            f"- Issues: {len(rows)}",
            f"- Range: KOFUN-{start_id:05d} through KOFUN-{end_id:05d}",
    ...
    for filename, name, count, start, end in index_rows:
        index.append(f"| [{filename}]({filename}) | {escape(name)} | {count} | KOFUN-{start:05d}–KOFUN-{end:05d} |")
    index += [
        "",
