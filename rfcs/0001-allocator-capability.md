...
- **Effect visibility (#556).**
  [`LANGUAGE_VISION.md`](https://github.com/hjosugi/kofun-site/blob/main/content/docs/LANGUAGE_VISION.md),
  in `hjosugi/kofun-site`,
...
1. **Allocation effect — "may allocate".** `alloc` is an inferred effect
   ...
   `LANGUAGE_VISION.md`. In the conceptual notation of
...
- On the managed default path, heap exhaustion is a runtime error `R030`
  ...
  `LANGUAGE_VISION.md`, which folds panics into `pure`: managed
...
`LANGUAGE_VISION.md` §Error messages. The codes extend the ownership
...
- **Second-class values.** Region tagging deliberately reuses the
  ...
  and `LANGUAGE_VISION.md` records that capabilities-as-second-class
...
- **Zig-style explicit allocator parameter on every allocating function, no
  ...
  `LANGUAGE_VISION.md` promise. Kofun's application layer must not pay
...
per
[`RFC_PROCESS.md`](https://github.com/hjosugi/kofun-site/blob/main/content/docs/RFC_PROCESS.md)
§6, in `hjosugi/kofun-site`.
