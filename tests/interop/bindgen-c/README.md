...
| the crossing is memory- and UB-clean | the emitted boundary and the fixture implementation are rebuilt together under AddressSanitizer and UndefinedBehaviorSanitizer with `-fno-sanitize-recover=all` and leak detection on, and must reproduce the same golden with empty stderr — the fixture is compiled in rather than linked as the shared object, so neither side of the crossing is left uninstrumented |
| the sanitizers are actually armed | three probes built with the same flags — a use-after-free, a leak, and a signed overflow — must each be diagnosed by name; a sanitizer that was never linked in is otherwise silent in exactly the way a clean run is |
