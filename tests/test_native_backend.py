...
from frost.native_backend import NativeBackend, compile_to_executable  # noqa: E402
from frost.c_backend import BackendFailure                   # noqa: E402
...
def run_interpreted(path: Path) -> str:
    result = subprocess.run(
        [sys.executable, "-m", "frost.cli", "run", str(path)],
        ...
        for name, source in PROGRAMS.items():
            with self.subTest(program=name):
                with tempfile.TemporaryDirectory() as tmp:
                    workdir = Path(tmp)
                    source_path = workdir / f"{name}.frost"
