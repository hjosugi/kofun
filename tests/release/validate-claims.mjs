...
// JSON Schema subset
//
// The schema stays authoritative rather than being restated in JavaScript, so
// this interprets the keywords `spec/release-claim.schema.json` actually uses.
// An unknown keyword is a hard error: silently ignoring one would let a
// constraint be written into the schema and never enforced.
// ---------------------------------------------------------------------------
...
function validateAgainstSchema(root, node, value, path, errors) {
