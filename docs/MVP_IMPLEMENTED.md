...
| x86-64/AArch64 native integer division | `//` and `%` use the specified floor semantics; `/` follows the executable seed's tracked truncating behavior (#687); all guard zero and non-representable quotients, with bounded `kofun:` diagnostics rather than canonical `R010` | `tests/conformance/functions`, `bootstrap/native/check.sh` |
