...
| x86-64/AArch64 native integer division | `//`, `/`, and `%` with the specified flooring, guarded zero divisor, and guarded non-representable quotient; runtime diagnostics are still the bounded `kofun:` form, not canonical `R010` | `tests/conformance/functions`, `bootstrap/native/check.sh` |
