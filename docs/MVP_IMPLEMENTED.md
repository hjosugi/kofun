...
| concrete enum matching with one `Int` payload | bounded Stage 2 C11 slice with constructor-set exhaustiveness; a constructor may carry one `Int` payload that a `C(name)` pattern binds and the arm body and guard read. Wider payload types, more than one field, nested payload patterns, and enum-typed parameters or returns stay outside it | `tests/conformance/syntax/issues_35_47/run.sh`, `tests/fuzz/enum_match.sh` | `enum-matching` |
