...
| compiled visibility interfaces | bounded Stage 2 KIF v1.1: exact resolved flat-nominal function/payload refs; public/internal/private leak rejection; atomic public/internal filtering | `task visibility-filtering`, `task visibility-api-leaks`, `task module-interface-artifact` | `compiled-visibility-interfaces` |
...
| exact Decimal and binary64 Float arithmetic | Stage 2 C11 lowers exact Decimal `+`, `-`, `*`, comparison/equality, and checked `/` with `Exact`/`InexactDivision`/`DivisionByZero`; Float remains observably binary64; every other declared backend has an explicit unsupported capability row (#723) | `tests/conformance/decimal-arithmetic`, `tests/conformance/capabilities.tsv`, `tests/conformance/decimal/run.sh` | `decimal-arithmetic-v1` |
