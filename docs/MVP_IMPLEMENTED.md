...
| x86-64/AArch64 native Int64 literals | full signed range in the narrowest encoding per value, making `INT64_MIN` and the non-representable-quotient trap reachable | `bootstrap/native/check.sh` |
| x86-64/AArch64 native integer division | `//` and `%` use the specified floor semantics; `/` follows the executable seed's tracked truncating behavior (#687); all guard zero and non-representable quotients, with bounded `kofun:` diagnostics rather than canonical `R010` (#690) | `tests/conformance/functions`, `bootstrap/native/check.sh` |
