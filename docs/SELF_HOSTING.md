...
Kofun compilerを「自己ホスト済み」と呼ぶための最低条件は次です。

1. Stage 0 compilerがKofunで書かれたStage 1 compilerをbuildできる。
...
| Stage 1 | `bootstrap/stage1/compiler.kf` | working native seed |
...
Stage 1 seedはKofunで書かれています。現在は次のKofun Core subsetをC11へ変換します。
...
```text
Stage 0 parses and type-checks bootstrap/stage1/compiler.kf
  -> Path A: Stage 0 interpreter runs the Kofun-written compiler
  -> Path B: Stage 0 C11 backend builds a native Stage 1 compiler
  -> both Stage 1 paths compile bootstrap/fixtures/answer.kf
...
| Label | Meaning |
|---|---|
| Kofun-written compiler seed | compiler codeの一部がKofunで動く |
| native Kofun-written compiler seed | Stage 0がKofun compiler seedをnative executableへbuild可能 |
| self-hosting frontend | frontend自身をKofunでbuild可能 |
| self-hosting compiler | Stage 1が自分自身をbuild可能 |
| fixed-point bootstrap | Stage 1/Stage 2 artifact equivalenceを確認済み |

このrepositoryの現在のlabelは**native Kofun-written compiler seed**です。Stage 1 self-recompileがopenなので、self-hosting compilerまたはfixed-point bootstrapとは呼びません。
