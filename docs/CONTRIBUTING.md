...
`editor/tree-sitter-kofun/grammar.js` is the authored grammar.
`src/parser.c`, `src/grammar.json`, and `src/node-types.json` are generated by
the pinned CLI and committed.

```sh
npm --prefix editor/tree-sitter-kofun ci
npm --prefix editor/tree-sitter-kofun run generate
...
Inspect generated parser changes; do not hand-edit only the generated files.
...
| the browser tour | `make tour` | `make verify` |
| docs Markdown | the `spec/*/check.sh` reading it, if any | `make verify`, plus `npm run test:docs` in `hjosugi/kofun-site` when the site renders it |
