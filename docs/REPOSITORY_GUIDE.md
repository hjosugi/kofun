...
|---|---|---|
| `release/claims.json` | `docs/MVP_IMPLEMENTED.md` as the sole `public_sources` entry, twelve documents as `specification`, and `docs/MEMORY_MODEL.md` and `docs/SECURITY.md` as `threat_model` | `make release-claims` |
| `rfcs/index.json` | `docs/DESIGN_DECISIONS.md` as a decision `source`, and four documents as `normative_spec` | `make rfc-registry` |
| `artifacts/release-evidence/index.json` | a SHA-256 of each of fourteen documents under `evidence_digests` | `make release-claims`, regenerated by `make release-evidence` |
| a gate script | `spec/*/check.sh`, `tests/**/run.sh`, `bootstrap/*/check.sh` and `docs/tour/check.sh` read documents directly | the owning `make` target |

Measured on this tree, of the thirty tracked `.md` files under `docs/`:

- **twenty-four are resolved by one of the four.** Moving any of them would not
...
git grep -oh -E 'docs/[A-Za-z0-9_./-]+\.md' -- \
    '*.sh' Makefile '*.mjs' '*.ts' '*.c' '*.kofun' '*.json'
```
...
So the four `docs/tour/guides/*.md` files read as free and are not: `make tour`
