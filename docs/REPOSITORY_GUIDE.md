...
Measured on this tree, of the twenty-eight tracked `.md` files under `docs/`:

- **twenty-five are resolved by one of the four.** Moving any of them would not
  lose prose, it would break a gate.
- **two are resolved only by `app/docs/docs-manifest.ts`** in
  [`hjosugi/kofun-site`](https://github.com/hjosugi/kofun-site), which renders
  `CONTRIBUTING.md` and `REPOSITORY_GUIDE.md` out of the submodule. They stay
  because they describe this source tree and its contribution gates.
