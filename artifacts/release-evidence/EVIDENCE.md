...
| `compiler-seed` | `sh bootstrap/stage1/check.sh` | The Kofun seed and audited C transliteration emit byte-identical C for the arithmetic, Bool, nested-block and loop corpora; the Bool, nested-block and loop programs execute with every zero-dividing skipped operand, condition and unentered loop body unevaluated. |
