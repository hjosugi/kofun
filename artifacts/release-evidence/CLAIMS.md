...
| `compiler-seed` | implemented | bootstrap | The Stage 1 seed is written in Kofun and kept beside a hand-audited C transliteration. Its line-oriented Core lowers checked Int arithmetic, six comparisons, Bool equality, Bool literals and bindings, unary `!`, short-circuiting `&&`/`||`, nested `if`/`else if`/`else` blocks whose bindings leave scope at their `}`, and `while`/`for NAME in A .. B` loops that nest with those blocks, evaluate each range end once, and scope the bound name to the loop body; `print` remains Int-only and the Core still has no assignment statement. |
