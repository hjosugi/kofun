...
| `compiled-visibility-interfaces` | checkpoint | frontend | The normal Stage 2 compiler publishes committed Int and flat nominal ADT parameter, result, and constructor-payload identities into canonical public and package-internal KIF views. Public facts may reference only public ADTs; internal facts may reference public or internal ADTs; private and absent identities fail closed. Records, generics, effects, and ownership signature components remain explicit unsupported-publication boundaries. |
| `compiler-seed` | implemented | bootstrap | The Stage 1 seed is written in Kofun and kept beside a hand-audited C transliteration. Its line-oriented Core lowers checked Int arithmetic, six comparisons, Bool equality, Bool literals and bindings, unary `!`, short-circuiting `&&`/`||`, Text literals/concatenation/equality, byte-oriented Text/List indexing, nested `if`/`else if`/`else` blocks, and `while`/half-open `for` ranges. The 15 builtins used by the frozen self-host source have exact arity and typed lowering: `args`, `chars`, `contains`, `find`, `is_digit`, `is_space`, `is_xid_continue`, `len`, `print`, `read_text`, `starts_with`, `text_slice`, `trim`, `validate_unicode_source`, and `write_text`; the Core still has no assignment or non-main function declaration. |
| `decimal-arithmetic-v1` | checkpoint | c-backend | Stage 2 C11 lowers exact arbitrary-precision Decimal addition, subtraction, multiplication, comparison and checked exact division, plus explicitly binary64 Float arithmetic. Int has no `/`, Decimal has no `//` or `%`, mixed numeric arithmetic requires an explicit conversion, and the other declared backends remain unsupported for this corpus. |
...
| `nominal-records` | checkpoint | frontend | The dedicated record frontend accepts the full bounded typed contract. The production Stage 2 C11 slice additionally lowers nominal records containing only Int/Bool fields, preserves written construction evaluation order, uses declaration-order AggregateLayout, supports typed field reads, and passes/returns whole records by value. Module visibility, generic records, wider field types, native lowering, and a stable ABI remain outside the checkpoint. |
