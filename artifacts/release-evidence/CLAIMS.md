...
| `enum-matching` | checkpoint | frontend | Concrete enums match exhaustively over their constructor set. A constructor may declare one `Int` payload field, constructed as `C(expr)` in an explicitly typed initializer and bound by a `C(name)` or `C(_)` pattern. Wider payload types, more than one payload field, nested payload patterns, generic enums, and enum-typed parameters or returns are outside the slice. |
