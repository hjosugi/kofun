...
| `compiler-seed` | rejection | `bootstrap/selfhost/driver/corpus_reject_else_without_if.kofun` | An `else` with no `if` to attach to exits nonzero and writes no C; twenty-eight adjacent typed-boundary, block-boundary, loop-boundary and Text-boundary fixtures are checked by the same gate. |
