...
| `compiler-seed` | rejection | `bootstrap/selfhost/driver/corpus_reject_else_without_if.kofun` | An `else` with no `if` to attach to exits nonzero and writes no C; twenty-two adjacent typed-boundary, block-boundary and loop-boundary fixtures are checked by the same gate. |
