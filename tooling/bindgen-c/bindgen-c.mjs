...
//
//   <module>.raw.kofun      raw trusted extern declarations in the checked
//                           C ABI profile surface (bootstrap/c_abi);
...
const TRUST = 'raw-trusted-foreign';
...
function parseArguments(argv) {
  ...
  if (!IDENTIFIER.test(options.module)) fail(`module name must be an identifier: ${options.module}`);
  return options;
}
...
function main() {
  ...
  push(`# ${options.module}.raw.kofun - GENERATED by \`kofun bindgen-c\`. DO NOT EDIT.`);
  push('#');
  push('# =====================================================================');
  ...
  push('#');
  push(`# audit report: ${options.module}.bindgen.json`);
  push('#');
  ...
  const modulePath = path.join(options.outDir, `${options.module}.raw.kofun`);
  const reportPath = path.join(options.outDir, `${options.module}.bindgen.json`);
