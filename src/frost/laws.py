...
from . import ast
from .diagnostics import Diagnostic, FrostError
...
class LawChecker:
    ...
    def _check_monad(
        ...
        for value in ordered_values:
            for fn in ordered_functions:
                ...
                except (FrostError, Exception) as error:  # normalized below
                    checked += 1
                    failures.append(
                        LawFailure(
                            "left identity",
        ...
        for monad in ordered_monads:
            ...
            except (FrostError, Exception) as error:
                checked += 1
                failures.append(
                    LawFailure(
                        "right identity",
    ...
    def _error_text(error: BaseException) -> str:
        if isinstance(error, FrostError):
